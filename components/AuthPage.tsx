...
export default function AuthPage() {
  ...
  const resetFields = () => { setEmail(""); setPassword(""); setUsername(""); setError(""); };
  ...
  const switchMain = (m: MainMode) => { setMainMode(m); setMode("login"); resetFields(); };
        ...
        <div style={{ background: "rgba(13,21,37,0.85)", border: `1px solid ${mainMode === "admin" ? "rgba(255,107,107,0.35)" : "rgba(108,99,255,0.3)"}`, borderRadius: "20px", padding: "2rem", backdropFilter: "blur(20px)", boxShadow: "0 20px 60px rgba(0,0,0,0.5)", transition: "border-color 0.3s" }}>
          {mainMode === "user" && (
            <div style={{ display: "flex", background: "rgba(255,255,255,0.04)", borderRadius: "10px", padding: "3px", marginBottom: "1.75rem" }}>
              {(["login", "register"] as const).map(m => (
                <button key={m} onClick={() => { setMode(m); setError(""); }} style={{ flex: 1, padding: "0.55rem", borderRadius: "8px", border: "none", cursor: "pointer", fontWeight: "600", fontSize: "0.88rem", transition: "all 0.2s", background: mode === m ? "linear-gradient(135deg, var(--accent), #8b7cf8)" : "transparent", color: mode === m ? "white" : "var(--text-muted)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            ...
            {mainMode === "user" && mode === "register" && (
              <div>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
                  {AVATARS.map(a => (
                    <button key={a} onClick={() => setAvatar(a)} style={{ width: "44px", height: "44px", borderRadius: "10px", border: `2px solid ${avatar === a ? "var(--accent)" : "var(--border)"}`, background: avatar === a ? "rgba(108,99,255,0.2)" : "rgba(255,255,255,0.03)", cursor: "pointer", fontSize: "1.4rem", transition: "all 0.2s", transform: avatar === a ? "scale(1.15)" : "scale(1)" }}>
                      {a}
                    </button>
                  ))}
                </div>
            ...
            <button className="btn-primary" onClick={handleSubmit} disabled={loading}
              style={{ width: "100%", justifyContent: "center", marginTop: "0.25rem", padding: "0.85rem", background: mainMode === "admin" ? "linear-gradient(135deg, #ff6b6b, #ee5a24)" : undefined, opacity: loading ? 0.7 : 1 }}>
              {loading ? "⏳ Please wait..." : mainMode === "admin" ? "🔐 Admin Sign In" : mode === "login" ? "🚀 Sign In" : "⚔️ Start Quest"}
            </button>
          </div>
        </div>
      </div>
